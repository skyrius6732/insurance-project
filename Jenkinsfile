        ...
        stage('Deploy') {
            steps {
                script {
                    ...
                    sh "docker run -d --name ${newContainerName} --network insurance-net -p ${newDeploymentPort}:8080 SPRING_KAFKA_BOOTSTRAP_SERVERS=kafka:9092 skyrius6732/insurance-project:latest"
